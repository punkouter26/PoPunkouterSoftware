...
- **AI triage:** `AiTriageService` (Infrastructure) calls the Hugging Face Inference API (`google/flan-t5-base`, free tier) to turn the dashboard's attention-item list into a one-paragraph summary. Killed by `FeatureFlags:EnableAiSummary` (off by default) and by the `ai-hf` typed `HttpClient` having deliberately **no** resilience pipeline — an AI outage must degrade to "AI summary unavailable", not retry-storm the free quota. Consumer: `POST /api/diag/ai`. This replaces the 2026-07 "no AI integration" decision (the old `azure-openai` client that backed it is still gone); do not re-add a *second* AI client speculatively.
